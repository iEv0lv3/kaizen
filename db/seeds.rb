...
@question_7 = @david.questions.create!({ subject: 'I totally forgot what I have to get done for PD this mod?', content: 'So I am a mod 2 student and I cannot remember what I had to get done this mod in PD, anyone know the link to the requirnments?', forum: 1, upvotes: 1 })
...
@question_15 = @jordan.questions.create!({ subject: 'Active record query help', content: 'I cannot figure out how to get this query where I need the count of something?', forum: 0, upvotes: 1 })
...
@answer_5 = @question_5.answers.create!({ content: 'just go to https://turing.io/login then just log into the page.', user_id: @sebastian.id, upvotes: 1 })
...
@answer_12 = @question_12.answers.create!({ content: 'So your cart module is really a session you need to pull in the data at the model level.', user_id: @jordan.id, upvotes: 1 })
...
@answer_14 = @question_14.answers.create!({ content: 'Well yeah you can, the whole point is to get employed ', user_id: @jordan.id, upvotes: 1 })
@answer_15 = @question_15.answers.create!({ content: 'Sure!! We should pair though message me in slack!! ', user_id: @steve.id, upvotes: 1 })
@answer_16 = @question_16.answers.create!({ content: ' The agile work flow is beimg more adaptive and less responsive to a situation', user_id: @david.id, upvotes: 1 })
@answer_17 = @question_10.answers.create!({ content: 'You can mock it without a Vcr to if you DM me on slack we can talk it out?', user_id: @steve.id, upvotes: 1 })
@answer_18 = @question_12.answers.create!({ content: 'Your Cart can be accessed if you pull in the data, I can pair with your tommorow in the T?', user_id: @david.id, upvotes: 1 })

...
RSpec.describe 'As a Visitor' do
  describe 'When I visit a questions show page' do
    before :each do
      ...
      @answer_3 = @question_2.answers.create!(
        { content: 'Because you are not trying. ',
          upvotes: 1,
          user_id: @user_2.id }
      )

      @answer_4 = @question_2.answers.create!(
        { content: 'Jobs are hard to come by, remember turing has a staff that you can reach out to to help you get there. hang in there.',
          upvotes: 1,
          user_id: @user_2.id }
      )
    end
    ...
    it 'and if the answer is one that I made then I can delete it' do
      ...
      within "#answer-#{@answer_1.id}" do
        within "#question-links-#{@answer_1.id}" do
